...
- `envlt get <key>` prints a single variable's current value, unmasked, to stdout, e.g. `export DB_PASSWORD=$(envlt get DB_PASSWORD)`. Unlike `vars`, which always masks `Secret` values, asking for a specific key by name is treated as an intentional reveal, the same way `gen --show` already worked
